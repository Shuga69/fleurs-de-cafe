function addToWishList(){
    ...
    let addWishMobileButtons = document.getElementsByClassName('add-to-wishlist-mobile')
    ...
    for(let item of addWishButtons){
        item.addEventListener("click",(event)=>addWishEvent(event))
}
for(let item of addWishMobileButtons){
    item.addEventListener("click",(event)=>addWishEvent(event))
}
}

const addWishEvent = (event)=>{
